...
struct stella_vslam_jll
{
  ...
  std::vector<double> processRGBD(cv::Mat image, cv::Mat depth, double timestamp);
  std::vector<double> processStereo(cv::Mat left, cv::Mat right, double timestamp);
private:
...
std::vector<double> stella_vslam_jll::processRGBD(cv::Mat image, cv::Mat depth, double timestamp)
{
    
    // input the current frame and estimate the camera pose
    std::shared_ptr<stella_vslam::Mat44_t> pose = (system->feed_RGBD_frame(image, depth, timestamp));
    
    std::vector<double> out;
	out.resize(pose->rows() * pose->cols());
	for(size_t i = 0; i < pose->rows(); i++)
    {
		for(size_t j = 0; j < pose->cols(); j++)
        {
...
std::vector<double> stella_vslam_jll::processStereo(cv::Mat left, cv::Mat right, double timestamp)
{
    
    // input the current frame and estimate the camera pose
    std::shared_ptr<stella_vslam::Mat44_t> pose = (system->feed_stereo_frame(left, right, timestamp));
    
    std::vector<double> out;
	
    out.resize(16);
    if (pose && pose->size() == 16)
    {
...
JLCXX_MODULE define_julia_module(jlcxx::Module& mod)
{
  ...
  .method("processRGBD", &stella_vslam_jll::processRGBD)
